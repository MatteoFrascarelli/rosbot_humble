...
def generate_launch_description():
    ...
    pkg_uav_single_gazebo = get_package_share_directory('uav_single_gazebo')

    ign_gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ros_ign_gazebo, 'launch', 'gz_sim.launch.py')),
        launch_arguments={
            'gz_args': '-v 4 -r trial_1.world --gui-config ~/.ignition/gazebo/6/gui.config'
        }.items(),
    )
    ...
    rviz = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', os.path.join(pkg_uav_single_gazebo, 'rviz', 'drone.rviz')],
